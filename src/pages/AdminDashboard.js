...
export default function AdminDashboard() {
  ...
  const [publicSelection, setPublicSelection] = useState([]); const [docTarget, setDocTarget] = useState(null); const [docDetail, setDocDetail] = useState(null); const [docSelection, setDocSelection] = useState([]); const [docLoading, setDocLoading] = useState(false);
  ...
  async function approveRequest(requestId) {
    ...
    await runAction(requestId, `/admin/requests/${requestId}/approve`, { adminRemarks: remarks, publicDocuments: publicSelection }, 'Campaign approved and creator notified.', () => { setViewRequest(null); setPublicSelection([]); });
  }
  ...
  function toggleDocument(url, list, setList) {
    setList((current) => current.includes(url) ? current.filter((item) => item !== url) : [...current, url]);
  }
  async function openDocumentManager(campaign) {
    if (!campaign?._id) return;
    setDocTarget(campaign); setDocDetail(null); setDocLoading(true); setError('');
    try {
      const response = await api.get(`/admin/campaigns/${campaign._id}`);
      const detail = response.data.campaign || response.data;
      setDocDetail(detail); setDocSelection(Array.isArray(detail.publicDocuments) ? detail.publicDocuments : []);
    } catch (requestError) {
      setError(requestError.response?.data?.message || 'Unable to load campaign documents.');
      setDocTarget(null);
    } finally {
      setDocLoading(false);
    }
  ...
  async function saveDocuments() {
    if (!docTarget) return;
    setWorkingId(docTarget._id); setNotice(''); setError('');
    try {
      await api.patch(`/admin/campaigns/${docTarget._id}`, { publicDocuments: docSelection });
      setNotice('Public documents updated.'); setDocTarget(null); setDocDetail(null);
      await load();
    } catch (requestError) {
      setError(requestError.response?.data?.message || 'Unable to update documents.');
    } finally {
      setWorkingId(null);
    }
  }
  ...
  return <section className="admin-page"><div className="admin-hero"><div><p className="admin-eyebrow">Administrator workspace</p><h1>Fundraising overview</h1><p>Review student requests and keep every campaign accountable.</p></div><button className="secondary-button" onClick={load}>Refresh</button></div>
    ...
    {docTarget && <div className="modal-backdrop" role="presentation"><div className="edit-modal request-detail-modal" role="dialog" aria-modal="true" aria-labelledby="doc-modal-title"><div className="modal-header"><div><p className="admin-eyebrow">Campaign documents</p><h2 id="doc-modal-title">Choose public documents</h2></div><button type="button" onClick={() => setDocTarget(null)} aria-label="Close document settings">×</button></div>{docLoading || !docDetail ? <p className="empty-admin">Loading documents…</p> : <div className="request-detail-body"><p className="request-detail-meta"><strong>{docDetail.title}</strong></p><p className="request-detail-description">Checked documents appear on the public campaign page so supporters can verify authenticity.</p>{Array.isArray(docDetail.documents) && docDetail.documents.length > 0 ? <div className="request-detail-section"><h4>Supporting documents</h4><ul className="request-detail-list">{docDetail.documents.map((document) => <li key={document} className="request-detail-doc"><label className="request-doc-toggle"><input type="checkbox" checked={docSelection.includes(document)} onChange={() => toggleDocument(document, docSelection, setDocSelection)} /><span>Show publicly</span></label><a href={document} target="_blank" rel="noreferrer">{document}</a></li>)}</ul></div> : <p className="empty-admin">No supporting documents were uploaded for this campaign.</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setDocTarget(null)}>Cancel</button><button className="approve" disabled={workingId === docTarget._id} onClick={saveDocuments}>{workingId === docTarget._id ? 'Saving…' : 'Save selection'}</button></div></div>}</div></div>}
  </section>;
}
